...
* author:crispliu@tencent.com

#### 概述
该项目为一个外部调用的方法，基于jquery1.X，里面传入四个默认参数 ```title```、```minVersion```、```channelNum```、```qrHref```，分别对应着网站的```名称```，```最小适配的ie型号```，```下载渠道号```，```二维码链接```

```javascript
var defaultOpts = {
                title: '网页',//标题
                minVersion:10,//最小适配ie,如果不传入默认支持到10
                 channelNum:"",//下载渠道号
                qrHref:'' //二维码链接
};
```
...
在启用低版本推荐计划的网站中，开头引入
```javascript
<script src='https://stdl.qq.com/stdl/qqbrowser/floatlayer/qb-recommend-float-layer.js?'+ new Date().getTime()></script>
```
#### 调用方法：
```$.qbRecommand({title:'QQ音乐',minVersion:10,channelNum:'122'});显示如下```
 ![image.png](/uploads/1CF4BCE80B9F4D5CAA53F3EC03AA8E78/image.png height="330" width="495")
 
 传入 qrHref:'' //二维码链接，比如：
```javascript
 $.qbRecommand({title:'QQ音乐',minVersion:10,channelNum:'122',qrHref:"https://stdl.qq.com/stdl/qqbrowser/floatlayer/img/yqqImg.png"})显示如下
```
![image.png](/uploads/26A141C9B21F4218BDB9D205ED173962/image.png)
